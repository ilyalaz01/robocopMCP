...
| 3 | Orchestrator + local pipeline | ☑ | AI | full sub-game + series over MCP (heuristic) |
...
- ☑ Per-mechanism PRDs filled with real content (Phase 0–10).

## Phase 11 — remaining for Ilya (see `_build/MORNING_BRIEF.md`)
- ☐ Live Gmail OAuth send (`credentials.json` + browser consent + `token.json`).
- ☐ Cloud deploy both MCP servers (public URLs + revocable token).
- ☐ Inter-team bonus match (exchange URLs/token; both email identical JSON).
- ☐ Create the GitHub remote and `git push`.
